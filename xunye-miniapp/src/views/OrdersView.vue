...
function statusText(status: string, serveStatus?: string) {
  if (status === 'PAID' && serveStatus === 'FINISHED')
    return '已完成'
  if (status === 'PAID')
    return '制作中'
  if (status === 'UNPAID')
...
const orderCards = computed<OrderCard[]>(() => rawOrders.value.map((order) => {
  ...
  return {
    ...
    status: statusText(order.status, order.serveStatus),
    amount: Number(order.totalAmount || 0),
    count: order.items.reduce((sum, item) => sum + Number(item.quantity || 0), 0),
    month: dateParts.month,
    week: Date.now() - new Date(order.createdAt).getTime() <= 7 * 24 * 60 * 60 * 1000,
...
const filteredOrders = computed(() => {
  let result = selectedFilter.value === 'all'
    ...
    : selectedFilter.value === 'week'
      ? orderCards.value.filter(item => item.week)
      : orderCards.value.filter(item => item.month === selectedFilter.value)
  ...
  if (selectedProduct.value !== 'all')
    result = result.filter(o => o.items.some(i => i.includes(selectedProduct.value)))
  return result
})
...
function chooseStatus(v: string) {
  selectedStatus.value = v
  closeFilterSheet()
}

function chooseTable(v: string) {
  selectedTable.value = v
  closeFilterSheet()
}

function chooseItemType(v: string) {
  selectedItemType.value = v
  selectedProduct.value = 'all'
  closeFilterSheet()
}

function chooseProduct(v: string) {
  selectedProduct.value = v
  closeFilterSheet()
}

function openOrder(order: OrderCard) {
  store.selectOrder(order.no)
  push('orderDetail')
}
</script>
...
<template>
  <view class="view">
    ...
    <scroll-view class="orders-list" scroll-y enhanced show-scrollbar="false">
      ...
      <view v-for="order in filteredOrders" :key="order.id" class="order-card" @tap="openOrder(order)">
    ...
    <view v-if="dateSheetOpen" class="sheet-mask" @tap="closeDatePicker">
      <view class="date-panel" @tap.stop>
        ...
        <view class="month-grid">
          <view
            v-for="month in months" :key="month.value" class="month-cell"
    ...
    <view v-if="filterSheet" class="sheet-mask compact-mask" @tap="closeFilterSheet">
      <view class="filter-panel" @tap.stop>
        ...
        <view v-else class="filter-options">
          ...
          <button v-for="opt in productOptions" :key="opt" class="filter-option" :class="{ active: selectedProduct === opt }" @tap="chooseProduct(opt)">
            <text>{{ opt }}</text><text v-if="selectedProduct === opt">✓</text>
          </button>
        </view>
      </view>
    </view>
...
.orders-head {
  ...
  gap: 12px;
  min-height: 44px;
  padding: calc(var(--xunye-safe-top, 44px) + 8px)
    calc(var(--xunye-menu-right, 0px) + var(--xunye-menu-width, 0px) + 12px) 12px 16px;
