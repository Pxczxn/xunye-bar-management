  ...
  <view class="view">
    ...
    <scroll-view class="content cart-content" scroll-y enhanced show-scrollbar="false">
      <view class="summary-card">
        ...
        <view>
          <view class="muted small">就餐桌台</view>
          <view class="bold">{{ store.currentTable?.code || '未选桌' }}</view>
        </view>
      </view>
      <view class="panel">
        <view class="panel-title">已选商品</view>
        <view v-if="!store.totalQty" class="empty-line">购物车空空如也</view>
      ...
      <view v-if="store.activeCoupon" class="panel">
        <view class="discount-line">
          <view>
            <view class="bold">{{ store.activeCoupon.title }}</view>
            <view class="muted small">{{ store.activeCoupon.rule }}</view>
          </view>
          <button class="plain-button gold" hover-class="none" @tap="store.removeCoupon">不用了</button>
        </view>
      </view>
    </scroll-view>
    <view class="bottom-pay">
      <view>
        <view v-if="store.discountAmount" class="muted small">已优惠 ¥{{ store.discountAmount.toFixed(2) }}</view>
        <view v-else class="muted small">合计</view>
        <view class="pay-total">¥{{ store.payableAmount.toFixed(2) }}</view>
      </view>
      <uv-button
        text="提交订单"
