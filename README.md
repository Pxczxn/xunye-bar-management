...
| staff | 123456 | STAFF（员工） | 订单列表/详情、点单POS、桌台查看；**不可**访问 Dashboard、商品、库存、员工管理 |
...
| 前端页面跳转到 /orders | 当前角色（STAFF）无权访问该页面 | 正常行为，STAFF 默认首页为订单页 |
...
| 3 | STAFF 账号登录 | 默认进入 /orders，侧边栏不显示 Dashboard/商品/库存/员工 |
| 4 | STAFF 手动输入 /dashboard | 自动重定向到 /orders |
| 5 | 小程序扫码选桌台 | 可浏览商品、加购物车、提交订单 |
| 6 | 后台收到新顾客订单 | 顶部出现"待处理顾客订单 N"橙色提醒，10秒内自动刷新 |
| 7 | 后台点击"开始制作" | 订单制作状态变为"制作中"，小程序订单详情同步显示"制作中" |
| 8 | 后台点击"完成订单" | 订单制作状态变为"已完成"，小程序显示"已完成"，桌台无其他未完成订单时自动释放 |
| 9 | 未支付订单（paid_at 为空） | 不计入 Dashboard 今日营收、客单价、销售趋势、支付方式统计；**今日订单数仍按 created_at 统计，包含未支付订单** |
| 10 | 已支付订单（status=PAID，paid_at 有值） | 正确计入 Dashboard 今日营收、订单数、支付方式统计、销售趋势 |
| 11 | 顾客端接口不带后台 Token | `/api/customer/**` 正常返回数据，不受后台鉴权影响 |
